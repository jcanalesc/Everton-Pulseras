...
class Popen:
  ...
  def _start_ctrl_c_wrapper(self, cmd):
    cmd_str = "start \"\" python winctrlc.py "+"\""+cmd+"\""+" "+str(self._port)+" "+str(self._logger)
...
class Wrapper:
  ...
  def __init__(self, cmd, port, fp):
    signal.signal(signal.SIGINT, self._signal_handler)
    subprocess.Popen(cmd, stdout=fp, stderr=fp)
...
if __name__ == "__main__":
  ...
  Wrapper(command_string, port_no, fp)